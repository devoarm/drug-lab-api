...
export const checkAuthByDate = async (req: Request, res: Response) => {
  ...
  const sql = `SELECT
	...
	a.ServiceDate as service_date,
	a.AuthenCode as authen_code
	FROM vn_stat v
	...
	LEFT JOIN (SELECT
	 ...
	 WHERE 
		CAST(AuthenDate AS DATE) = "${data.vstdate}") AS a ON p.cid = a.CID
 WHERE       
	v.vstdate = '${data.vstdate}'
	${
    data.timeStart == "00:00:00" && data.timeEnd == "00:00:00"
      ? ``
      : `AND TIME(a.ServiceDate) BETWEEN '${data.timeStart}' AND '${data.timeEnd}'`
  }  
